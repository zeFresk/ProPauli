...
TYPED_TEST_SUITE(CircuitRun, tested_policies_t);

TYPED_TEST(CircuitRun, simple_run) {
	Circuit qc{ 1 };
	qc.add_operation("I", 0);
	auto res = qc.run({ "I" }, this->policy);
	EXPECT_FLOAT_EQ(res.expectation_value(), 1.f);
}

TYPED_TEST(CircuitRun, qc_match_observable_result_1) {
	Circuit qc{ 4,
	...
	EXPECT_EQ(qc.run(obs, this->policy), evolved_obs);
...
TYPED_TEST(CircuitRun, qc_merge_works_with_scheduler) {
	Circuit qc{
	...
	for (unsigned i = 0; i < 4; ++i) {
		...
		qc.add_operation("rz", 0, -1.f);
		EXPECT_EQ(qc.run({ "Z" }, this->policy).size(), 1);
	}
}

TYPED_TEST(CircuitRun, qc_truncate_works_with_scheduler) {
	Circuit qc{ 1,
	...
	for (unsigned i = 0; i < 4; ++i) {
		...
		EXPECT_EQ(qc.run({ "Z" }, this->policy), Observable<coeff_t>("Z", std::pow(1.f - noise, i + 1)));
	}
}

TYPED_TEST(CircuitRun, qc_update_truncator) {
	Circuit qc{ 4,
		    std::make_unique<NeverTruncator<>>(),
		    {},
		    std::make_unique<AlwaysAfterSplittingPolicy>(),
	...
	auto res = qc.run({ "ZZZZ" }, this->policy);
...
TYPED_TEST(CircuitRun, test_circuit1) {
	/* qreg q[4]; creg c[4];
	h q[0];
	h q[1];
	h q[2];
	...
	for (auto const [ob, ev] : truth_table) {
		auto res = qc.run(Observable{ ob }, this->policy);
		EXPECT_NEAR(res.expectation_value(), ev, 1e-4f);
	}
}

TYPED_TEST(CircuitRun, test_circuit1_batch) {
	/* qreg q[4]; creg c[4];
	h q[0];
	h q[1];
	h q[2];
...
TYPED_TEST(CircuitRun, test_circuit1_splitter_rd) {
	Circuit qc{ 4 };
	...
	for (auto const& [o_str, ev] : truth_table) {
		auto o = Observable{o_str};
		auto [res, splt] = qc.run_rdkn_split_err(o, 1, this->rpolicy);
		auto res_evd = res.ev_detailed();
		auto splt_evd = splt.ev_detailed();
