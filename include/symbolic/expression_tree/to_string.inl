		...
		[&](auto const& n) -> std::string {
			...
			if constexpr (std::is_same_v<VT, Constant<T>>) {
				...
				if (n.operation == UnaryOp<T>::Op::Minus) {
					bool is_simple =
						std::get_if<Constant<T>>(&n.exp->node_type) || std::get_if<Variable>(&n.exp->node_type);
				...
				case BinaryOp<T>::Op::Add:
					return (is_root) ? std::format("{} + {}", lhs_str, rhs_str) :
							   std::format("({} + {})", lhs_str, rhs_str);
					...
					for (size_t i = 1; i < n.operands.size(); ++i) {
						...
						if (term != nullptr && coeff < 0) {
							...
							if (positive_coeff == 1) {
								...
								res += to_string_node(process_multiplication(mul_ops), fmt, false);
							}
						} else if (term == nullptr && coeff < 0) { // Negative constant term
							auto v = -coeff;
							res += " - " + std::vformat(fmt, std::make_format_args(v));
						} else if (term == nullptr && coeff >= 0) { // Positive term
							res += " + " + std::vformat(fmt, std::make_format_args(coeff));
						} else {
							res += " + " + to_string_node(term, fmt, false);
						}
					}
